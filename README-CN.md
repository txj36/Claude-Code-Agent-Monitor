...
| **通知** | 会话开始、完成、错误和子 Agent 生成的浏览器通知。可按事件配置开关及权限管理 |
...
Dashboard 支持原生浏览器通知，在你未查看 Dashboard 标签页时提供实时提醒。

### 工作原理

1. **启用** — 在设置页面通过主开关启用通知
2. **授权** — 在浏览器提示时授予权限（Web Notifications API 要求）
3. **配置** — 选择哪些事件触发通知：
...
- **偏好设置**存储在 `localStorage` 的 `agent-monitor-notifications` 键下
- **`useNotifications` hook** 在应用根级别（`App.tsx`）订阅 WebSocket 事件总线，根据保存的偏好触发 `new Notification()` 调用
- **权限管理**在设置页面中处理，带有已授权/已拒绝/待提示的可视化指示器
- **测试通知**按钮在设置页面中，让你验证设置是否正常工作
- 无服务端组件 — 通知完全在客户端，由 WebSocket 消息触发
